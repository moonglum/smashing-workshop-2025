export async function renderServerComponents(jsx: any): Promise<any> {
  if (
    typeof jsx === "string" ||
    typeof jsx === "number" ||
    typeof jsx === "boolean" ||
    ...
    return Promise.all(jsx.map((child) => renderServerComponents(child)));
    ...
    if (jsx.$$typeof === Symbol.for("react.element")) {
      if (typeof jsx.type === "string") {
        ...
        return {
          ...jsx,
          props: await renderServerComponents(jsx.props),
        };
        ...
        return renderServerComponents(returnedJsx);
      ...
      return Object.fromEntries(
        await Promise.all(
          Object.entries(jsx).map(async ([propName, value]) => [
            propName,
            await renderServerComponents(value),
          ])
        )
      );
    }
